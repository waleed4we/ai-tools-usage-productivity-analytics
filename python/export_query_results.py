"""
Export all numbered PostgreSQL analysis-query results into ONE CSV.
...
Each query's result is kept as its own block. Because the 40 queries
have different columns, missing columns are left blank.
"""
...
# ============================================================
# 1. DATABASE SETTINGS — CHANGE THESE
# ============================================================
...
# ============================================================
# 2. THE 40 ANALYTICAL QUERIES
# ============================================================

QUERIES = [
    ('1.1', 'Row counts across every table', """SELECT 'Ai Tools' AS table_name, COUNT(*) AS total_records FROM ai_tools
UNION ALL
...
SELECT 'Payments', COUNT(*) FROM payments;"""),
    ('1.2', 'Users overview: total users, countries, professions, experience levels', """SELECT
...
FROM users;"""),
    ('1.3', 'AI tools overview: total tools, categories, pricing models', """SELECT
    COUNT(tool_id) AS total_tools,
    COUNT(DISTINCT category) AS total_categories,
    COUNT(DISTINCT pricing_model) AS pricing_models
FROM ai_tools;"""),
    ('2.1', 'Users by experience level', """SELECT experience_level, COUNT(*) AS number_of_users
FROM users
GROUP BY experience_level
ORDER BY number_of_users DESC;"""),
    ('2.2', 'Users by profession', """SELECT profession, COUNT(*) AS number_of_users
FROM users
GROUP BY profession
ORDER BY number_of_users DESC;"""),
    ('2.3', 'Users by country', """SELECT country, COUNT(*) AS number_of_users
FROM users
GROUP BY country
ORDER BY number_of_users DESC;"""),
    ('2.4', 'Number of AI tools per category, highest to lowest', """SELECT category, COUNT(*) AS number_of_tools
FROM ai_tools
GROUP BY category
ORDER BY number_of_tools DESC;"""),
    ('2.5', 'Pricing model distribution with percentage of total tools', """-- (fixed: cast to numeric BEFORE dividing, otherwise Postgres does integer division and truncates to 0)
SELECT
    pricing_model,
...
ORDER BY percentage DESC;"""),
    ('2.6', 'User distribution by country with percentage of total users', """-- (fixed: same integer-division issue as 2.5)
SELECT
    country,
...
ORDER BY users_percentage DESC;"""),
    ('2.7', 'User distribution by experience level with percentage of total users', """-- (fixed: same integer-division issue as 2.5)
SELECT
    experience_level,
...
GROUP BY experience_level
ORDER BY percentage DESC;"""),
    ('2.8', 'Signups per year', """SELECT
    EXTRACT(YEAR FROM created_at) AS signup_year,
    COUNT(*) AS total_signups
FROM users
GROUP BY EXTRACT(YEAR FROM created_at)
ORDER BY signup_year ASC;"""),
    ('2.9', "Top 5 countries by user count, NULLs relabeled as 'Unknown'", """SELECT
...
LIMIT 5;"""),
    ('3.1', 'All free-tier AI tools', """SELECT * FROM ai_tools WHERE pricing_model = 'Free';"""),
    ('3.2', 'Freemium tools in the Coding or Research categories', """SELECT tool_name, category, pricing_model
FROM ai_tools
WHERE pricing_model = 'Freemium'
  AND category IN ('Coding', 'Research');"""),
    ('4.1', 'Unique users and total usage records per tool', """SELECT
    ait.tool_name,
...
ORDER BY unique_users DESC;"""),
    ('4.2', 'Total usage metrics (sessions, prompts, tokens) per tool category', """SELECT
    ai_tools.category,
...
ORDER BY total_tokens DESC;"""),
    ('4.3', 'Registered users who have never used any AI tool', """SELECT users.user_id, users.country, users.profession
FROM users
LEFT JOIN usage_log ON users.user_id = usage_log.user_id
WHERE usage_log.user_id IS NULL;"""),
    ('4.4', 'Unique users and usage records per category (with category ranking)', """SELECT
    ai_tools.category,
...
GROUP BY ai_tools.category
ORDER BY ranking;"""),
    ('5.1', 'Most common experience level within each profession (ties included)', """WITH experience_counts AS (
    SELECT
        profession,
        experience_level,
        COUNT(*) AS number_of_users,
        DENSE_RANK() OVER (
            PARTITION BY profession ORDER BY COUNT(*) DESC
        ) AS ranking
    FROM users
...
WHERE ranking = 1
ORDER BY number_of_users DESC;"""),
    ('5.2', 'AI adoption rate per experience level', """-- (% of users in each experience level who've used at least one AI tool)
SELECT
    u.experience_level,
...
ORDER BY ai_adoption_percentage DESC;"""),
    ('5.3', 'Users whose total token consumption exceeds the average across all users', """WITH user_tokens AS (
    SELECT
        users.user_id,
        users.profession,
...
ORDER BY total_tokens_used DESC;"""),
    ('5.4', 'Most-used AI tool per profession, ranked by total prompts submitted', """WITH ranking_cte AS (
    SELECT
        usage_log.tool_id,
        ai_tools.tool_name,
        users.profession,
        ...
        DENSE_RANK() OVER (
            PARTITION BY users.profession ORDER BY SUM(usage_log.prompts) DESC
        ) AS ranking
    FROM usage_log
    JOIN ai_tools ON ai_tools.tool_id = usage_log.tool_id
    JOIN users ON users.user_id = usage_log.user_id
...
ORDER BY prompts_used DESC;"""),
    ('5.5', 'Full per-tool summary: unique users, total tasks, total revenue from active subs', """WITH tool_usage AS (
    SELECT
        tool_id,
...
LEFT JOIN tool_revenue r ON t.tool_id = r.tool_id;"""),
    ('5.6', 'Which plan each profession purchases the most', """WITH profession_and_plan_cte AS (
    SELECT
        users.profession,
        subscriptions.plan,
        COUNT(subscriptions.subscription_id) AS total_purchases,
        DENSE_RANK() OVER (
            PARTITION BY users.profession ORDER BY COUNT(subscriptions.subscription_id) DESC
        ) AS ranking
    FROM users
...
WHERE ranking = 1;"""),
    ('6.1', 'Active subscriptions created in 2025', """SELECT subscription_id, user_id, plan, monthly_price, start_date
FROM subscriptions
WHERE EXTRACT(YEAR FROM start_date) = 2025
  AND status = 'Active'
ORDER BY start_date;"""),
    ('6.2', 'Total revenue from successful payments per AI tool category', """SELECT
    ai_tools.category,
    SUM(payments.amount) AS total_revenue
FROM ai_tools
JOIN subscriptions ON ai_tools.tool_id = subscriptions.tool_id
JOIN payments ON subscriptions.subscription_id = payments.subscription_id
WHERE payments.payment_status = 'Success'
GROUP BY ai_tools.category
ORDER BY total_revenue DESC;"""),
    ('6.3', 'Users who completed more than 500 tasks in total', """WITH user_activity_cte AS (
    SELECT
        users.user_id,
        users.profession,
...
ORDER BY total_tasks_completed DESC;"""),
    ('6.4', 'Users currently on an Active Freemium-tool subscription', """SELECT DISTINCT users.user_id, users.country
FROM users
JOIN subscriptions ON users.user_id = subscriptions.user_id
JOIN ai_tools ON ai_tools.tool_id = subscriptions.tool_id
WHERE ai_tools.pricing_model = 'Freemium'
  AND subscriptions.status = 'Active';"""),
    ('6.5', 'Payments categorized by value tier (High / Medium / Low)', """SELECT
    CASE
...
GROUP BY payment_category;"""),
    ('6.6', 'Users signed up in 2025 with more than 50 total sessions', """WITH conditional_cte AS (
    SELECT
        users.user_id,
        users.profession,
        users.country,
...
WHERE total_sessions > 50;"""),
    ('6.7', 'Monthly Recurring Revenue (MRR) by plan, from active subscriptions', """SELECT
    subscriptions.plan,
...
ORDER BY monthly_revenue DESC;"""),
    ('6.8', 'Count of active subscriptions', """SELECT COUNT(*) AS active_subscriptions
FROM subscriptions
WHERE status = 'Active';"""),
    ('6.9', 'Count of cancelled subscriptions', """-- (simplified: chk_subscription_status_dates guarantees status = 'Cancelled' <=> end_date IS NOT NULL,
--  so checking status alone is sufficient and avoids a redundant OR condition)
SELECT COUNT(*) AS cancelled_subscriptions
FROM subscriptions
WHERE status = 'Cancelled';"""),
    ('6.10', 'Users with more than 2 tool categories used', """WITH category_breadth_cte AS (
    SELECT
        users.user_id,
        users.profession,
...
WHERE cat_count >= 2;"""),
    ('7.1', 'Most popular AI tool (by unique users)', """SELECT
    ai_tools.tool_name,
...
LIMIT 1;"""),
    ('7.2', 'Least used AI tool (by unique users)', """WITH ranking_cte AS (
    SELECT
        ai_tools.tool_name,
        COUNT(DISTINCT usage_log.user_id) AS total_unique_users,
...
WHERE ranking_in_least_used = 1;"""),
    ('7.3', 'Profession that uses AI the most', """SELECT
    users.profession,
    COUNT(DISTINCT usage_log.user_id) AS unique_users
FROM users
JOIN usage_log ON users.user_id = usage_log.user_id
...
ORDER BY unique_users DESC
LIMIT 1;"""),
    ('7.4', 'Profession that uses AI the least', """SELECT
    users.profession,
...
LIMIT 1;"""),
    ('7.5', 'Most commonly used pricing model', """SELECT
...
ORDER BY number_of_users DESC
LIMIT 1;"""),
    ('7.6', 'Least used pricing model', """SELECT
    ai_tools.pricing_model,
    COUNT(DISTINCT usage_log.user_id) AS number_of_users
FROM ai_tools
JOIN usage_log ON ai_tools.tool_id = usage_log.tool_id
...
def main():
    ...
    try:
        for query_no, query_name, sql in QUERIES:
            ...
            try:
                ...
                if df.empty:
                    # Preserve the fact that this query ran but returned no rows.
                ...
                # Keep going so one failed query does not stop the other 39.
    ...
    # Different queries return different columns.
    # pandas automatically aligns them and leaves unavailable cells blank.
